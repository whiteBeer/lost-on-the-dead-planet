...
export function checkCircleRotatedRectangleCollision(
    ...
    const transCircle = rotate(distX, distY, -rect.rotation);

    const halfWidth = rect.width / 2;
    const halfHeight = rect.height / 2;
    const closestX = Math.max(-halfWidth, Math.min(halfWidth, transCircle.x));
    const closestY = Math.max(-halfHeight, Math.min(halfHeight, transCircle.y));

    // 4. Проверяем расстояние от этой точки до центра круга (в локальных координатах)
    const distanceX = transCircle.x - closestX;
    const distanceY = transCircle.y - closestY;
